...
/**
 * Border-glow effect that follows the cursor when hovering near a card.
 * Renders a conic-gradient ring whose angle tracks pointer position.
 */
  ...
  useEffect(() => {
    ...
    const handleMove = (e?: MouseEvent | { x: number; y: number }) => {
      ...
      animationFrameRef.current = requestAnimationFrame(() => {
        ...
        element.animate(
          { ["--start" as string]: `${newAngle}` },
          { duration: movementDuration * 1000, fill: "forwards", easing: "cubic-bezier(0.16,1,0.3,1)" }
        );
      });
    };
